                    ...
                    for r in resp:
                        ...
                        for m in window:
                            if r_ack == m["seqno"]:
                                ...
                                # Handle RTT
                                rtt = r["recv_time"] - m["send_time"]
                                rtt_history.append(rtt)
                                if len(rtt_history) > self.window_size:
                                    rtt_history = rtt_history[1:]
                                avg_rtt = sum(rtt_history) / float(len(rtt_history))
                                self.timeout = float(avg_rtt * 2) + float(0.01) #May need to fix val
